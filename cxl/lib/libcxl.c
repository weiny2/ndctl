...
static void *add_cxl_memdev(void *parent, int id, const char *cxlmem_base)
{
	...
	sprintf(path, "%s/dc/region_count", cxlmem_base);
	if (sysfs_read_attr(ctx, path, buf) < 0)
		memdev->dc_region_count = 0;
	else
		memdev->dc_region_count = strtoull(buf, NULL, 0);

	for (int region = 0; region < memdev->dc_region_count; region++) {
		sprintf(path, "%s/dc/region%d_size", cxlmem_base, region);
		if (sysfs_read_attr(ctx, path, buf) < 0)
			goto err_read;
		memdev->dc_size[region] = strtoull(buf, NULL, 0);
	}
...
{
	if (index >= 0 && index < NUM_DC_REGIONS)
